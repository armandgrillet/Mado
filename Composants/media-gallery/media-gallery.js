...
function updateSelection(e) { // Clic sur une image dans la liste
...
function scanGallery(entries) { // Chargement des images
...
window.addEventListener("load", function() {
   ...
   document.getElementById('gallery-button').addEventListener("click", function() { // Charge les galeries
      chrome.mediaGalleries.getMediaFileSystems({
         interactive : 'if_needed'
      }, getGalleriesInfo);
   });
   ...
   document.getElementById('scan-button').addEventListener("click", function () { // Charge les images dans un tableau
      // Reset.
      clearContentDiv();
      clearList();

      if (gGalleryArray.length > 0) {
         scanGalleries(gGalleryArray[0]); // Chargement
      }
   });
